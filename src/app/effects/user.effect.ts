...
export class UserEffects {
  ...
  @Effect() init$: Observable<Action> = this.actions$
    ...
    .map(action => {    
      ...
      // }
      return new user.InitilizedAction({loginStatus:status, currentAccount:null});
    })

  @Effect() login$: Observable<Action> = this.actions$
    .ofType(user.ActionTypes.LOGIN)
    // .map(action => new user.LoginResultAction({loginStatus:LoginStatus.LoggedIn}))
    ...
    .map(account => {
      if(account){
              ...
              return new user.LoginResultAction({loginStatus:LoginStatus.LoggedIn, currentAccount: account});
            }else{
              return new user.LoginResultAction({loginStatus:LoginStatus.LoggingFailed, currentAccount: null});
            }
    })
    .catch(() => Observable.of(new user.LoginResultAction({loginStatus:LoginStatus.LoggingFailed, currentAccount: null})));
  ...
  @Effect() loginResult$: Observable<Action> = this.actions$
    ...
    .map(action => {
      ...
      if(status == LoginStatus.LoggedIn){
        let role = action.payload.currentAccount.role;
        if(role == "管理员"){
